...
function getPropertyHtml(propArray = [placeholderPropertyObj]) {
    let cardHTML = ""
    propArray.map(prop => {
        console.log(prop)
        const {image, priceGBP, propertyLocation, comment} = prop
        const area = prop.roomsM2.reduce((total, area) => total + area)
        cardHTML += `
        <section class="card">
